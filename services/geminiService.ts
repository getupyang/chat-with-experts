...
export const fetchExperts = async (topic: string, language: Language): Promise<Expert[]> => {
  ...
  const actionName = 'fetch_experts';
...
export const fetchReplacementExpert = async (
  ...
  const startTime = Date.now();
  const actionName = 'fetch_replacement_expert';
...
export const fetchDebateResponse = async (
  ...
  const startTime = Date.now();
  const actionName = 'fetch_debate_response';

  const expertProfiles = experts.map(e => `- ${e.name} (${e.title}): ${e.reason}. Speaking Style: ${e.style}`).join("\n");
  
  // Convert previous messages to a format the model understands contextually
